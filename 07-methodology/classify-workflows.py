...
def main():
    ...
    # VS folder-slug lookup (so generated links resolve to real folders)
    vs_folder = {}
    for d in glob.glob(os.path.join(WF, "VS-*")):
        mv = re.match(r"VS-(\d+)-", os.path.basename(d))
        if mv:
            vs_folder[int(mv.group(1))] = os.path.basename(d)
    ...
    lines = []
    lines.append("# Proposed Criticality Classification (keyword-driven)")
    lines.append("")
    ...
    for tier, label in [(1, "Tier 1 — Core / statutory / revenue-critical (proposed)"),
        ...
        for wnum, name, vsnum in rows:
            slug = vs_folder.get(vsnum, f"VS-{vsnum}")
            name_clean = name.replace("|", "\\|")
            lines.append(f"| {wnum} | {name_clean} | [VS-{vsnum:02d}]({slug}/README.md) |")
        lines.append("")
