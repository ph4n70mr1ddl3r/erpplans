...
validator — 76 checks, run in CI and before every publish) enforces, at minimum, these check
classes:
...
the reference corpus grew it to 76 checks, each one the scar tissue of a real defect.
...
| [`validate-repo.sh`](validate-repo.sh) | The reference consistency engine (76 checks) behind §8.5 |
