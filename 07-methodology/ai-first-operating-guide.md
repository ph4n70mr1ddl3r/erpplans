...
validator — 71 checks, run in CI and before every publish) enforces, at minimum, these check
classes:
...
the reference corpus grew it to 71 checks, each one the scar tissue of a real defect.
...
| [`validate-repo.sh`](validate-repo.sh) | The reference consistency engine (71 checks) behind §8.5 |
