...
- auto-review (invoice (pi) review**: receive pi from)
- auto-send (of credit; sends l/c to vendor)
- auto-import (production completion; import coordinator coordinates with)
...
### Automation Opportunity
- auto-calculate ((w2b); system calculates risk transfer point)
- auto-import (fob, cfr), import coordinator issues "marine)
- auto-track (monitoring**: system tracks shipment location vs.)
...
### Automation Opportunity
- auto-calculate (authority api; calculates free-time expiration date)
- auto-track (**clearance tracking**: import coordinator monitors)
- auto-log (expiry; alerts logistics manager to prioritize)
...
### Automation Opportunity
- auto-log (a designated logical/physical cbw location in)
- auto-generate (request is generated.)
- rule-based auto-approval (boc approves, and the wms)
...
### Automation Opportunity
- auto-calculate (e2m system; calculates duties/taxes (w239))
- auto-document (**documentary review**: boc verifies)
- auto-inspect (**physical inspection**: if flagged (red/yellow)
...
### Automation Opportunity
- auto-review (pipeline status review**: import coordinator reviews)
- auto-document (issues: missing documentation, customs hold, examination)
- auto-validate ((>2 days): verify all documentation submitted;)
...
| **Volume** | ~5,000–7,000 import entries/year across all import vendors |
...
- **HS code misclassification risk** — incorrect HS code leads to either overpayment of duties (reducing margin) or underpayment (triggering BOC audit, penalties of 40–100%, and potential blacklisting); automated classification reduces but does not eliminate this risk for new or unusual items
...
### Automation Opportunity
- auto-document (advance shipping documents from vendor/freight forwarder:)
- auto-review (customs broker review; (d) system calculates)
- auto-calculate ((a) system calculates cif value: invoice)

### Controls
operational: downtime** — the BOC e2m system experiences periodic downtime
### Staffing Implication
...
- Per entry: 3–6 hours from document receipt to BOC filing. Monthly: ~400–600 entries × 4 hours avg = ~1,600–2,400 hours/month (shared across Customs Broker and Import Specialists). Duty reconciliation: ~6 hours/month.
...
### Automation Opportunity
- auto-review (import coordinator reviews all open import)
- auto-validate (breakdown; (e) verify container loading plan:)
- auto-route (for the route; (b) compare fcl)
