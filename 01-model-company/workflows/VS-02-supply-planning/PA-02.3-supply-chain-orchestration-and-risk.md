...
- auto-route (costs by route)
- auto-capture (**modeling**: use "center of gravity" and)
- rule-based auto-approval (plan**: if approved, trigger w116 (site)
...
### Automation Opportunity
- auto-dispatch (**shipment dispatch**: 3pl partner dispatches)
- auto-track (**milestones tracking**: control tower system)
- auto-calculate (system dynamically recalculates estimated time of)
...
### Automation Opportunity
- auto-scan (loading & scan**: dispatch crew loads)
- auto-dispatch (**customer dispatch alert**: system automatically)
- auto-calculate (system dynamically recalculates eta based on)
...
### Automation Opportunity
- auto-assign (procurement director assigns risk assessment to)
- auto-log (vendor; (d) **logistics risk** — shipping)
- auto-calculate (composite score calculated with configurable weights:)
...
### Automation Opportunity
- auto-log ((vendor-to-dc), import logistics (port/customs/brokerage per w464),)
- auto-review (supply chain reviews cost-per-unit metrics: cost)
- auto-route (per kg, route efficiency (deliveries per)
...
### Automation Opportunity
- auto-log (dashboard review**: logistics manager reviews carrier)
- auto-document (incorrect loading, documentation error; (iii) external:)
- auto-review (decisions: (a) review upcoming 48-hour dispatch)
...
### Automation Opportunity
- auto-escalate (c-items); (c) escalate to vp supply)
- auto-log (procurement manager, logistics manager, finance representative;)
- auto-count (b2b key accounts; (d) activate demand)
...
### Automation Opportunity
- auto-compile (— system compiles carrier performance data)
- auto-generate (— system generates weekly scorecard per)
- auto-log (review — logistics manager reviews scorecards;)
...
### Automation Opportunity
- auto-import (time, (b) important — limited sources,)
- auto-calculate (category: system calculates vendor concentration using)
- auto-review (onboarding (financial review, tin verification), (b))
...
### Automation Opportunity
- auto-assign (**daily order-to-store assignment**: system groups approved)
- auto-log (optimization run**: logistics planner runs route)
- auto-review (**route plan review and adjustment**: logistics)
...
- Address geocoding engine (Google Maps API + PSA PSGC hierarchy from MDM-310)
...
### Automation Opportunity
- auto-count (circles) to account for road network,)
- auto-log (fleet (buildright logistics): 10-wheeler wing vans)
- auto-capture ((a) customer enters delivery address; system)
...
### Automation Opportunity
- auto-flag (erp; system flags all stores within)
- auto-count (affected store count and baseline demand;)
- auto-allocate (disaster skus; allocates available stock to)
...
- **Price gouging regulatory risk**: during declared calamities, the Philippine DTI strictly monitors pricing of essential construction materials; BuildRight must maintain SRP even as wholesale costs may increase due to supply scarcity


### Automation Opportunity
- auto-review (planning manager reviews historical typhoon demand)
- auto-track (using pagasa track forecast, (ii) for)
- auto-post (**post-storm damage assessment and)
...
- Total annual: ~250–500 hours/year
...
| **Volume** | ~400–600 TEUs/month across all import vendors; ~30–40 containers/week requiring port processing |
...
### Automation Opportunity
- auto-calculate (commodity type; calculate total cbm and)
- auto-review (import manager reviews consolidation opportunities: (a))
- auto-forecast (port congestion forecast, freight rate competitiveness,)

### Controls
operational: downtime, or typhoon-related port closures
### Time Estimate
- Weekly consolidation planning: 3–4 hours
...
- **Working capital constraint**: forward buying ties up significant working capital (a PHP 30M forward buy of steel represents ~3 days of total COGS); during tight cash periods, forward buy opportunities may be missed
...
### Automation Opportunity
- auto-review (commodity price review**: category managers compile)
- auto-import (capacity constraints, import restrictions), (d) recommend)
- auto-calculate (category manager calculates forward buy quantity)
...
### Automation Opportunity
- auto-log (planning and logistics; (b) review historical)
- auto-track (on pagasa track forecast; (c) for)
- auto-allocate (emergency stock allocated to stores proportionally;)

### Controls
operational: Supply Chain Director review and approval gate
### Staffing Implication
- Supply Chain Director: ~1–2 days/year on seasonal planning + ~4–8 hours per typhoon event × ~10 events = ~50–80 hours/typhoon season. Primary responsibility.
...
- Per typhoon event: 4–8 hours (Supply Chain Director) + 1–2 days (Merchandise Planner)
