...
- auto-forecast (runs automated forecast engine: statistical algorithms)
- auto-review (demand planner reviews forecast exception report:)
- auto-capture (flagged forecasts manually: overrides algorithm, inputs)
...
### Automation Opportunity
- auto-generate (demand planner generates seasonal forecast: prior-year)
- auto-review (category manager reviews seasonal forecast; identifies)
- auto-count (negotiates volume discounts, early-order incentives, and)
...
### Automation Opportunity
- auto-review (**demand review**: demand planner and)
- auto-validate (meet demand; check vendor lead times)
- auto-log (if high-cost logistics (w66) are needed)
...
### Automation Opportunity
- auto-log (daily temperature logs archived for quality)
- auto-validate (**inbound quality check (temperature-sensitive)**: at dc)
- auto-route (for multi-drop routes: load sensitive goods)
...
### Automation Opportunity
- auto-log (based on analogous store profiles but)
- auto-review (store ramp-up**: review analogous store profile)
- auto-document (self-approves with documentation; (ii) overrides affecting)
...
### Automation Opportunity
- workflow notification (with customer contact details for notification)
- auto-flag (shipments, (d) flag dc stock for)
- auto-post (customer-facing signage posted, (c) store manager)
...
### Automation Opportunity
- auto-review (triggers readiness review process; store operations)
- auto-capture (gl cost center created, bank deposit)
- auto-document (and backroom; documents findings with photos)
...
### Automation Opportunity
- auto-generate (planning analyst generates report per store)
- auto-identify (store identification**: identify stores with: (a))
- auto-sort (w130; (c) assortment change — new)
...
### Automation Opportunity
- auto-log (store manager logs emergency sourcing request)
- auto-document (equivalent quality documentation may be used;)
- auto-review (with post-event review; (f) vendor delivers)
...
### Automation Opportunity
- auto-sort (rainy season assortment planning**: (a) april)
- auto-generate (region and generates emergency push allocation)
- auto-count (w109: rapid count (box count verification,)
...
### Automation Opportunity
- auto-capture (tropical cyclone enters philippine area of)
- auto-calculate (zones — calculates expected demand surge)
- auto-generate (planner generates pre-positioning recommendation: additional)
...
### Automation Opportunity
- auto-submit (impact demand; submits event notification via)
- auto-log (uses historical analogs from similar past)
- auto-generate (system generates adjusted replenishment parameters)
...
### Automation Opportunity
- auto-review (**demand review**: demand planner presents)
- auto-forecast (to statistical forecast)
- auto-import (po status, import container pipeline, vendor)
...
*Workflow Count: 13 · Back to **[VS-02: Supply Planning](./README.md)** · [Value Stream Index](../value-stream-index.md)*
