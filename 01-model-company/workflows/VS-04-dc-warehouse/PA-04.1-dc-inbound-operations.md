...
- auto-log (vs. appointment log)
- auto-scan (scan/verify each item against)
...
### Automation Opportunity
- auto-generate (— system auto-generates assembly order when)
- auto-validate (component quantities; checks component availability at)
- auto-alert (dc supervisor alerts buyer for expedited)
...
### Automation Opportunity
- auto-route (based on route schedule, driver qualifications)
- auto-record (before departure; records in system via)
- auto-review (fleet manager reviews fuel consumption per)
...
### Automation Opportunity
- auto-route (carrier and route: (a) **ro-ro ferry**)
- auto-create (system creates transfer order per)
- auto-import (import coordinator books shipment)
...
### Automation Opportunity
- auto-match (**pre-allocation matching**: system evaluates incoming)
- auto-dispatch (outbound store dispatch doors)
- auto-scan (& direct scan**: forklift operator unloads)
...
### Automation Opportunity
- auto-scan (lease records; scan-registers container number in)
- auto-assign (**yard slot assignment**: yms automatically assigns)
- auto-update (yms to update the real-time yard)
...
### Automation Opportunity
- auto-count (daily cycle count task list by)
- auto-scan (using rf scanner; records count quantity;)
- auto-post (quantity; adjustment posted with cycle count)
...
### Automation Opportunity
- auto-forecast (seasonal volume forecast per dc (units,)
- auto-count (labor requirement (headcount, duration, shift pattern)
- auto-schedule (wms operation), schedule alignment with surge)
...
### Automation Opportunity
- auto-document (appointment and documentation (delivery receipt, packing)
- auto-validate (visual inspection: checks for transit damage)
- auto-inspect (qc inspector performs aql sampling)
...
| **Frequency** | ~400–600 TEUs/month across all DCs; ~100–150 containers/DC/month; ~4–7 containers/DC/day |
...
- **Container seal tampering risk** — broken or tampered customs seals indicate potential pilferage during transit; if detected, the entire container must be inventoried count-by-count against the packing list, delaying devanning by hours
...
### Automation Opportunity
- auto-log (receiving clerk logs container arrival in)
- auto-assign (**dock assignment and staging area)
- auto-document (number against documentation one final time,)
...
| 4 | **Rapid receiving & quality check**: Inbound truck arrives; DC Receiving Lead performs expedited receiving: (a) scan ASN barcode per W3B, (b) visual quality inspection (no visible damage, correct packaging), (c) pallet count verification against ASN; for cross-dock A-items, quality sampling reduced to 5% (vs. standard 15%) based on historical vendor quality score per W44 | DC Receiving Lead | DC Operations Supervisor | 30–60 min/truck |
...
### System Touchpoints
- Cross-dock eligibility engine with ABC classification and open order matching (W1279.1)
- Inbound-outbound allocation and routing optimization (W1279.2)
- WMS dock door management with cross-dock lane assignment
...
- **Quality bypass risk** — reduced quality sampling (5% vs. 15%) means defective goods may reach stores; mitigated by restricting cross-dock to high-reliability vendors (quality score >95% per W44)
...
### Automation Opportunity
- auto-inspect (require quality inspection hold per w681)
- auto-generate (replenishment orders; generates cross-dock allocation plan:)
- auto-route (vehicle for route); dc receiving lead)
...
| **Volume** | ~20–30 cross-dock pallets/containers per DC per day; ~80–120 cross-dock operations/day chain-wide |
| **Owner** | DC Operations Manager |
...
### System Touchpoints
- WMS cross-dock module — pre-arrival planning, sort plan generation, real-time sortation tracking
...
### Automation Opportunity
- auto-generate ((b) system generates cross-dock sort plan:)
- auto-scan (receiving: (i) scan asn barcode on)
- auto-route (each outbound route: dc dispatch coordinator)
...
| **Frequency** | ~6,000 DC goods receipts/month across 4 DCs; ~80% of vendors with portal access submit ASN before delivery |
...
### Automation Opportunity
- auto-submit (vendor submits asn via vendor)
- auto-match (system auto-matches asn to open)
- auto-scan (to rapid scan-based receiving upon truck)
...
### Automation Opportunity
- auto-record ((multiple angles), records damaged sku, quantity,)
- auto-count ((salvageable at discount), moderate (usable with)
- auto-update (staging area; updates inventory status to)
