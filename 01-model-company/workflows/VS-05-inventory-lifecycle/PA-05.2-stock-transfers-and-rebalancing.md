...
| **Frequency** | ~30–40 inter-DC transfers/month; ~700–1,300 store-to-store transfers/month (W204 customer-transfers ~50–100/week + W214 inter-store ~20–30/day) |
...
- Customer-initiated inter-store transfers absorb transport cost as a customer service — with ~20–30/day (W214), the aggregate monthly courier cost of PHP 120,000–450,000 is not tracked (per W214: PHP 200–500/trip) against store profitability.
...
- **DC Receiving**: adds ~15–30 min per return shipment to existing receiving workload. With ~100–200/month ÷ 4 DCs = ~25–50/DC/month. Manageable.

### Staffing Implication (W22 overall)
- Inter-DC transfers are part of Supply Planner's existing duties (within the 40-person Supply Chain & Logistics team (per profile §3.3)).
...
### Automation Opportunity
- System auto-forecast of "After promo approval" (replaces manual Step 1).
...
### Controls
- CTL-254 (ensure controlled execution — Stock Transfers & Rebalancing (PA-05.2).)
...
### Automation Opportunity
- System auto-identify of "Identify SKUs" (replaces manual Step 1).
...
- **3PL courier cost**: PHP 500–2,000 per trip × ~50–100 trips/week = PHP 100,000–800,000/month. Budgeted within regional logistics cost center.
- **No incremental headcount.**
...
- System auto-generate of "Order & Payment" (replaces manual Step 2).
...
- System auto-track of "tracks" (replaces manual Step 5).
...
- **DC Staff (source and destination)**: ~4 hours combined per transfer for loading and receiving. With ~30–40 transfers/month across 4 DCs (the inter-DC canon), this is ~30–40 hours/DC/month. Absorbed.
...
| **Volume** | ~30–40 transfer orders/month averaging 5–20 lines per transfer (per W22); total ~150–800 transfer lines/month |
| **Owner** | Supply Planning Manager |
...
### System Touchpoints
- Inter-DC transfer recommendation engine: automated rebalancing suggestions based on DC inventory levels and demand (W22)
- Transfer order management: create, approve, ship, receive workflow with full audit trail (W3)
- In-transit inventory tracker: real-time visibility into items between DCs with vessel/truck tracking integration (W106)
- Transfer reconciliation module: monthly reconciliation of all transfer orders with exception reporting (W22)
- Integration with W22 (inter-DC transfers — existing), W3 (DC receiving), W106 (DC dispatch), W66 (inter-island logistics), W4 (store replenishment — source of demand data), W31 (demand forecasting — trigger for rebalancing)
...
### Automation Opportunity
- System auto-record of "Daily catch-weight sales recording" (replaces manual Step 1).
- System auto-calculate of "recalculates WAC per catch-weight SKU per store" (replaces manual Step 2).
- System auto-count of "Weekly catch-weight cycle count" (replaces manual Step 3).
...
### Time Estimate
- Daily catch-weight sales recording: 3–5 min/transaction × ~30–60 catch-weight transactions/store/day × 200 stores ≈ ~18,000–60,000 min/day (absorbed in POS operations)
...
### Automation Opportunity
- System auto-review of "Stockout alert & assessment" (replaces manual Step 1).
- System auto-route of "Surplus DC identification" (replaces manual Step 2).
