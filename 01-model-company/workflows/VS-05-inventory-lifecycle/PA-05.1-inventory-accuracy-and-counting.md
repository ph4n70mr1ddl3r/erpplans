...
| 3 | Physically count each SKU in assigned section; enter quantity into device (4 Stock Associates each count ~175 SKUs/day, taking ~40 min each at ~10 sec/SKU) | Stock Associate | Department Supervisor | ~40 min per associate |
...
### Time Estimate
Daily count per Stock Associate: ~40 min for ~233 SKUs. Blind recount: 15–30 min additional for flagged items. Adjustment processing: 5–15 min per adjustment (including approvals). Total per store per day: ~2–3 hours across 4 Stock Associates (counting + recounts + adjustments).

### Staffing Implication
- **4 Stock Associates per store (per profile §12.1)**: Each counts ~175 SKUs/day (~40 min; 700 ÷ 4 associates), with remainder of time on replenishment, receiving, damage reporting, and BOPIS picking. The count of 4 (per §12.1) provides slack for absenteeism and BOPIS picking duties.
