...
- auto-calculate (system calculates replenishment needs per)
- auto-review (supply planner reviews suggested orders each)
- auto-route (and delivery route)
...
### Automation Opportunity
- auto-count (generates daily count assignment by section/aisle)
- auto-capture (assigned section; enter quantity into device)
- auto-flag (system count; flags variances)
...
### Automation Opportunity
- auto-flag (system flags consignment items in)
- auto-record (system records consignment receipt as)
- auto-scan (floor; pos scans barcode normally)
...
### Automation Opportunity
- auto-validate (sales associate checks real-time inventory across)
- auto-count (details (loyalty account or trade account),)
- auto-import ((c) if import item — shows)
...
### Automation Opportunity
- auto-count (from completed counts against configurable thresholds:)
- auto-scan (wrong sku scanned) → immediate correction)
- auto-match (error (quantity mismatch at gr); (b))
...
| **Frequency** | Monthly |
| **Volume** | ~300 consignment SKUs from 12 key vendors across 200 stores; ~60,000 SKU-location consignment records |
...
### Automation Opportunity
- auto-generate (by store; generates "sold consignment report")
- auto-count (**physical count reconciliation**: for top)
- auto-scan (sales (pos scan miss), (b) damage/defect)

### Controls
operational: attaching POS transaction detail as evidence in Step 4; operational: records, vendors may dispute settlement quantities; operational: distinct consignment SKU numbering and receiving protocol in W23
...
- **AP Clerk**: ~6 hours/month (12 vendors × 30 min settlement processing). Absorbed within existing role.
...
- **Manual shrinkage checking effort** — daily physical checking of top 20 consignment SKUs across 200 stores = ~2,000 store-hours/month; stores may deprioritize this during busy periods, increasing risk of undetected shrinkage


### Automation Opportunity
- auto-generate (w574, system generates daily consignment sales)
- auto-review (office clerk reviews consignment report as)
- auto-log (confirmed shrinkage: log as consignment loss)

### Controls
operational: on-hand is understated, creating apparent shrinkage

### Time Estimate
- Daily per store: ~15–25 minutes (report review 5–10 min + shrinkage check 10–15 min + confirmation 2 min)
- Weekly per store: ~5 hours/month on daily reconciliation × 200 stores = ~1,000 hours/month chain-wide
- Weekly vendor reporting (HQ): ~1–2 hours/week
- Monthly reconciliation preparation (HQ): ~2–3 hours/month
- **Total annual**: ~12,000–15,000 store-hours/year + ~80 HQ hours/year
...
| **Frequency** | Triggered — approximately 5–10% of daily cycle counts produce actionable variances at 4 DCs; ~15–30 discrepancy investigations/month |
| **Volume** | ~60–120 DC cycle count discrepancy investigations/year; recurring discrepancy patterns affect ~50–100 SKU-locations |
...
### Pain Points / Risks
- **Investigation backlog**: at 5–10% of daily cycle counts producing actionable variances, 4 DCs generating ~1,000 counts/day could produce 50–100 investigation alerts/day; Inventory Control Clerks can process ~15–20 investigations/day, creating a persistent backlog during peak periods
...
### Automation Opportunity
- auto-count (cycle count completes (w6); system)
- auto-alert (receives investigation alert with details: sku,)
- auto-match (if recount matches original physical count:)
...
### Automation Opportunity
- auto-record (actual dimensions recorded and board foot)
- auto-count (yard cycle count**: weekly: (a) system)
- auto-log ((e) findings logged in system for)

### Controls
operational: must clearly distinguish nominal (for ordering) from actual (for pricing and inventory)

### Staffing Implication
- Lumber Yard Associate: ~90 min/week on cycle count + 20 min/day bundle monitoring + ~45 min/receipt verification × ~20 receipts/month = ~20 hours/month. Absorbed within existing lumber yard staffing.
