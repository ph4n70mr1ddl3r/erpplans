...
| 64 | **Drug-Free Workplace & Substance Abuse Program** | ~6,757 employees across 5 entities/~205 sites plus ~10–20% contingent labor and a large **safety-sensitive** population (forklift/reach operators, delivery drivers under LTFRB/DOTC, paint/chemical handlers) inside a premises-liability-exposed customer environment (VS-147) — DOLE D.O. 53-03 and RA 9165 Art. V make a drug-free workplace program a legal obligation (non-compliance → DOLE finding/penalty + officers' personal RA 9165 liability), and an impaired operator/driver is a catastrophic safety risk | **New gap (Pass 16)** — 'drug test', 'drug-free workplace', 'substance abuse', 'reasonable suspicion', and 'random drug testing' each appeared in **zero** PA files as dedicated workflow headers — sprinkled as single steps inside VS-83 (clinic/exam), VS-19.1 (pre-employment onboarding), VS-24 (post-incident), VS-06.2 (driver), VS-22 (DOLE documentation), VS-56 (partner driver) — no value stream owned the *end-to-end drug-free workplace program* | **FILLED — VS-150** |
