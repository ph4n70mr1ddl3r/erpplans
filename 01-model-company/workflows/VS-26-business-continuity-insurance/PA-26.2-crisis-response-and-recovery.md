...
- auto-track (bc manager tracks pagasa bulletins every)
- auto-capture ((lumber, garden center), sandbag entrances in)
- auto-send (security alarm, send staff home with)
...
### Automation Opportunity
- auto-notify (communication — notify all it staff,)
- auto-validate (last replication checkpoint (target rpo: 15)
- auto-monitor (monitor dr site performance)
...
### Automation Opportunity
- auto-log (regional office; log closure reason and)
- auto-validate (per shutdown checklist, (e) secure all)
- auto-count (reopening, staff headcount confirmed safe, cash)
...
### Automation Opportunity
- auto-notify ((single store/system); notify appropriate response team)
- workflow notification (store it contact)
- auto-log (in incident log)
...
### Automation Opportunity
- auto-identify (scope — identify affected pos (in-transit,)
- rule-based authorization (backup vendors, authorized substitute skus, alternative)
- auto-allocate (is constrained, allocate available inventory to)
...
### Automation Opportunity
- auto-review (weekly strategy reviews)
- workflow notification (sanitation stations, contact tracing, and isolation)
- auto-track (availability — track employee health status)

### Controls
operational: CEO / VP Operations review and approval gate
...
| **Volume** | Each major activation reaches 6,757 employees, 200 Store Managers, 4 DC Managers, and potentially 600,000 loyalty customers |
...
### Automation Opportunity
- auto-review (legal counsel reviews for liability; ceo)
- auto-post ((c) intranet posting, (d) department head)
- auto-update (regular status updates at defined intervals)
...
### Automation Opportunity
- auto-review (schedule post-incident review — within 5)
- auto-identify (identify root causes —)
- auto-document (document lessons learned —)
...
- Facility maintenance work order system per W508
...
- **Staff safety during response** — staff attempting to save merchandise during flooding risk electrocution, drowning, and contamination exposure; evacuation must take priority over asset protection
- **Repeated flooding at same locations** — stores in chronic flood-prone areas may flood multiple times per year; long-term solution is facility elevation or relocation per W20


### Automation Opportunity
- auto-document (move cash, documents, and safe above)
- auto-capture (if water enters store: (a) store)
- auto-generate ((d) system generates damage report with)
...
| **Participants** | Regional Manager, Store Manager, all store staff, VP Operations, LP Officer, Facilities Maintenance, Supply Chain Director |
...
| 5 | **During typhoon**: Regional Manager maintains contact with Store Managers via battery-powered group chat (Viber/WhatsApp); Store Managers report: (a) staff safety status, (b) structural damage observed (if safe to assess), (c) flooding status; VP Operations monitors overall situation and coordinates with Supply Chain Director on DC operations | Regional Manager / Store Managers | VP Operations | Continuous |
...
- Store status dashboard: Open / Typhoon Closure / Yellow / Red status for all 200 stores visible to VP Operations and Regional Managers
...
### Automation Opportunity
- auto-send (stores: (a) sends automated notification to)
- auto-validate (stacks), (b) check roof drainage channels)
- auto-generate (w535, (e) prepare store for early)
...
- **Per store: ~150–200 staff-hours for full prepare-assess-reopen cycle**
...
| **Owner** | Regional Manager |
| **Participants** | Regional Manager, Store Manager, Maintenance/Utility, VP Operations, Facilities Manager, Safety Officer |
...
### Automation Opportunity
- auto-validate (**initial safety check-in**: once storm signal)
- auto-submit (each area; submits via mobile app)
- auto-review (regional manager reviews assessment and classifies)
