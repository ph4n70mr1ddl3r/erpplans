...
- **Employee data privacy awareness**: with 6,715 employees across 200 stores, ensuring consistent privacy practices at the store level is challenging; mitigated by annual mandatory training (Step 7) and simple POS-level procedures for customer data handling
...
Under Philippine tax law (BIR Revenue Regulations No. 5-2022 and related issuances), businesses using computerized accounting systems (CAS) — including ERP systems, POS systems, and ecommerce platforms — must register their system with the Bureau of Internal Revenue (BIR) before use. BuildRight Depot's unified ERP system (covering GL, AP, AR, POS, inventory, payroll, and ecommerce across 5 legal entities and 205 locations) constitutes a CAS that requires: (a) initial registration with the BIR Revenue District Office (RDO) where each entity is registered, including submission of system documentation, flowcharts, sample outputs, and sworn attestation; (b) annual compliance to ensure the system continues to meet BIR requirements for: (i) books of account registration (computerized books in lieu of manual books), (ii) registered POS/invoicing (each POS terminal must be BIR-registered with a unique serial number), (iii) e-invoicing readiness (BIR is phasing in mandatory e-invoicing for large taxpayers), (iv) data retention (7-year retention of electronic records per NIRC); (c) audit readiness for BIR CAS examination — BIR may send a team to verify that the registered system matches what is actually in use and that system controls are adequate. Non-compliance penalties: PHP 25,000–50,000 per unregistered system component, plus potential suspension of business operations. This workflow manages CAS registration, ongoing compliance, and audit readiness.
...
- Data retention system — 7-year electronic record archive and retrieval
