...
- auto-count (ratio of discount transactions)
- auto-generate (system generates daily exception summary)
- auto-review (lpo reviews daily exception report;)
...
- **Staff welfare check coverage**: HR must verify safety of ~6,757 employees across affected regions within 24 hours; if cellular networks are down (common after typhoons), welfare checks default to physical visits that may take days to complete
...
### Automation Opportunity
- auto-document (requirements, required documents, fee schedule (based)
- auto-alert (system alerts regulatory officer 60)
- auto-inspect (fire safety inspection certificate (bfp), (g))
...
### Automation Opportunity
- auto-monitor (sales rep monitors government procurement opportunities:)
- auto-match ((a) product match — does buildright)
- auto-document (prepares bid/quote documents per agency requirements:)
...
### Automation Opportunity
- auto-submit (employee submits grievance or whistleblower)
- auto-assign (24 hours; assigns case number; classifies)
- auto-record (explanation; system records date/time received; hr)
...
| **Store Operations** | Director, Regional Managers, CS Manager, Ops Standards, Facilities Coordinator | ~24 | W5, W5D (in-store delivery), W5E (opening delay), W16, W29, W5G (offline recovery), W34, W37, W41, W47, W49, W54A, W67, W69, W71 | ✅ 4 Regional Managers × 50 stores each; oversee new openings and monthly store performance reviews; in-store delivery uses existing 3PL carrier infrastructure; opening delay procedure is Store Mgr responsibility; offline recovery is Store Mgr responsibility; shift scheduling and complaint handling absorbed; 2–3 LPOs recommended for loss prevention; 1 Facilities Coordinator manages store maintenance, disaster response, and physical security oversight; weekly price compliance audits absorbed by Dept. Supervisors and Stock Associates |
...
| **Other** | Legal, Internal Audit, DPO, Regulatory Officer, Customer Service (call center), Executive | ~58 | W41 (complaints), W42 (audit observation), W53 (data privacy breach, DSAR lifecycle, data portability/erasure), W54 (LGU permits), W54A (BIR CAS registration), W62 (vendor contracts) | ✅ Support functions; call center handles multi-channel complaint intake; Legal expanded to include DPO (W53 with full DSAR lifecycle) and Regulatory Officer (W54 LGU + W54A BIR CAS); total Legal & Compliance expands from ~5 to ~7 |
...
| Cashiers | 3 | W5B (checkout), W17 (loyalty scan), W28 (gift card sell/reload) | 3 terminals; covered in shifts; tight on coverage |
...
### Automation Opportunity
- auto-record (number; system records hwgr number, registration)
- auto-document (submits renewal documents to denr-emb; renewed)
- auto-collect (or dedicated collection boxes, (d) **used)
...
### Automation Opportunity
- auto-capture (backup data center; verify data integrity)
- auto-validate (notification tree; verify response times from)
- auto-log (manual inventory logging for 4 hours;)
...
### Automation Opportunity
- auto-count (returns, employee discounts) via automated exception)
- auto-record (matches system record)
- auto-log (**discrepancy logging**: if "phantom return")
...
### Automation Opportunity
- auto-review (**promo review**: marketing submits promo)
- auto-generate (regulatory officer prepares and files permit)
- auto-track (**payment & tracking**: finance pays dti)
...
### Automation Opportunity
- auto-validate (**compliance checklist**: regulatory officer generates)
- auto-review (audit team reviews current social media)
- rule-based auto-approval (price" vs. "dti-approved promo price" for)
...
### Automation Opportunity
- auto-collect (**data collection**: collect quarterly data)
- auto-report (**report preparation**: pco consolidates)
- auto-review (**review & approval**: regulatory)
...
### Automation Opportunity
- auto-file (**bir notification**: file "notice of closure")
- auto-count (the computerized accounting system (cas) at)
- auto-notify (**statutory updates**: notify sss, philhealth, and)
...
### Automation Opportunity
- auto-submit (application**: store submits "special event permit")
- auto-inspect (**fire safety inspection**: bureau of fire)
- rule-based auto-approval (**permit display**: approved temporary permits posted)
...
### Automation Opportunity
- auto-file (import coordinator files icc/soc application via)
- auto-document (**documentary review**: dti-bps reviews)
- auto-inspect (**inspection & sampling**: upon)
...
| **Volume** | ~6,757 employees across 200 stores |
| **Owner** | Store Manager |
...
### Automation Opportunity
- auto-test ((x-ray, stool/drug test) at accredited clinic)
- auto-compile (hr assistant compiles medical results and)
- auto-inspect (files for inspection)
...
### Automation Opportunity
- auto-compile (store manager compiles proof of legitimate)
- auto-file (**submission**: file application with the)
- auto-inspect (**inspection**: regional officers inspect)
...
### Automation Opportunity
- auto-validate (extinguisher weight checks; contractor issues a)
- auto-document (store/dc personnel, documents it with photographs)
- auto-compile (safety officer compiles the fsic renewal)
...
### Automation Opportunity
- auto-schedule (expiry, pco schedules mandatory environmental testing:)
- auto-report (lab/stack test reports: (a) if compliant:)
- auto-record (water billing records (to calculate discharge)
...
### Automation Opportunity
- auto-flag (category managers flag chemical, paint, and)
- auto-assign (**technical representative assignment**: regulatory compliance manager)
- auto-validate ((w236) to verify compliance with fda)
...
### Automation Opportunity
- auto-validate (path proximity check**: during site feasibility)
- auto-generate (survey and prepare a certified geodetic)
- auto-document (**documentary package compilation**: project)
...
### Automation Opportunity
- auto-count (gathering**: tax accountant prepares bir branch)
- auto-document (submit complete documentary requirements; (c) attend)
...
### Automation Opportunity
- auto-capture (officer maintains enterprise risk register in)
- auto-report ((b) incident reports (per w559, w140),)
- auto-validate (department head validates likelihood and impact)
...
### Automation Opportunity
- auto-validate (be sold; spot-checks 10 random stores)
- auto-report (store-level confirmation reports)
- auto-calculate (notification published; calculates notification effectiveness rate)
...
### Automation Opportunity
- auto-distribute (analysis) refresh: distributes bia questionnaire to)
- auto-report (from direct reports; compliance manager consolidates)
- auto-validate (critical dependencies), validate mtd/rto/rpo targets against)
...
### Automation Opportunity
- auto-review (transaction monitoring review**: compliance officer reviews)
- auto-log ((e) system logs str filing with)
- auto-record (investigation techniques, record retention; (d) new)
...
### Automation Opportunity
- auto-validate (method (cash, check, online), required forms;)
- auto-review (tax manager reviews computed tax per)
- auto-record (lgu; (d) record payment in ap)
...
### Automation Opportunity
- auto-count (at csr counter per w271; (b))
- auto-log (**request logging**: dpo receives and)
- auto-validate (assesses: (a) check active transactions —)
...
- **Employee data privacy awareness**: with 6,757 employees across 200 stores, ensuring consistent privacy practices at the store level is challenging; mitigated by annual mandatory training (Step 7) and simple POS-level procedures for customer data handling
...
### Automation Opportunity
- auto-update (processing inventory update**: dpo updates the)
- auto-document (risks, (b) documents mitigation measures, (c))
- auto-record (of cctv recording; flags any consent)
...
- **LGU processing variability** — each LGU has different processing times, requirements, and bureaucratic complexity; a permit that takes 2 days in one LGU may take 3 weeks in another; no standardization across ~100 LGUs
...
### Automation Opportunity
- auto-compile (compliance officer compiles renewal calendar: (a))
- auto-validate (**pre-renewal compliance check** (december): for each)
- auto-document (**batch document preparation**: legal &)

### Controls
operational: standard operating procedures; system-enforced validation rules
### Staffing Implication

- Legal & Compliance: ~2–3 people dedicated to permit management during January–February peak; absorbed by existing ~9-person Legal & Compliance team. Store Managers: ~2 hours/year each for permit filing. No incremental headcount.

### Time Estimate

- Annual renewal cycle: ~800–1,000 hours total (preparation + filing + follow-up + posting across 205 locations). Peak months (December–January): ~400–500 hours/month. Distributed across Legal team and Store Managers.
...
### Automation Opportunity
- auto-document (system description document: erp modules, data)
- auto-log (maintain change log documenting all cas)
- auto-record (retention: electronic records for past 10)
...
### Pain Points / Risks
- **LGU fragmentation**: 205 locations across potentially 100+ different LGUs, each with unique forms, fees, and procedures; no standardized national process creates massive administrative complexity
...
- **Local political risk**: Philippine LGU officials change after elections (every 3 years); new leadership may change procedures, fees, or enforcement intensity; Store Managers must maintain LGU relationships through political transitions
- **Store closure risk**: failure to renew business permit can result in LGU closure order; at BuildRight's scale (200 stores), even a single day closure costs ~PHP 840,000 in lost revenue per store


### Automation Opportunity
- auto-document (compile required documents per lgu: (i))
- auto-file (10 locations): filed in person at)
- auto-calculate ((a) system calculates quarterly local business)
...
- **Volume scalability**: ~5–10M transactions/month is a significant transmission volume; the accredited service provider must handle this throughput without degradation
...
### Automation Opportunity
- auto-document ((f) all documentation filed for bir)
- auto-generate (pos system generates electronic sales invoice)
- auto-log (reviews failure log and resolves outstanding)
