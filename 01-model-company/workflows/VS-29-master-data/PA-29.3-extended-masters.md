...
- auto-log ((physical, commercial, logistical, compliance).)
- auto-report (impact: (a) reporting — new attributes)
- auto-review (data manager reviews the template change,)
...
### Automation Opportunity
- auto-review (definition & review**: merchandise planner proposes)
- auto-validate (system. system validates that all 200)
- auto-sort (**assortment matrix definition**: category)
...
### Automation Opportunity
- auto-count (type (percentage discount, fixed amount off,)
- auto-weigh ((clearance items, catch-weight items). system runs)
- auto-document (contribution is documented and the net)
...
### Automation Opportunity
- auto-count (write-off, cycle count variance, damaged goods),)
- auto-review (**rationalization review**: master data analyst)
- auto-capture (correct cost center bears the impact,)
...
### Automation Opportunity
- auto-create (item (already created per w252), component)
- auto-validate (data analyst validates the bom: (a))
- auto-count (analysis**: cost accountant calculates the kit)
...
### Automation Opportunity
- auto-count (manufacturer/parent company, country of origin, associated)
- auto-record (existing brand records using fuzzy matching)
- auto-validate (multiple brands. validates that linked vendors)
...
### Automation Opportunity
- auto-create (merchandise planner creates or updates a)
- auto-calculate (years. system calculates: (a) actual vs.)
- auto-import (time adjustments (import po cutoff dates),)

### Controls
- CTL-57 (ensure replenishment parameter governance.)
operational: to plan based on normal demand during peak periods
...
### Automation Opportunity
- auto-assign (gs1. for vendor-assigned barcodes, the system)
- auto-record (gtin/ean/upc is recorded as-is (verified against)
- auto-scan (the barcode scanned at pos for)
...
### Automation Opportunity
- auto-review (finance controller reviews the financial impact)
- auto-weigh (cards, services, catch-weight items below minimum,)
- auto-log (**redemption catalog governance**: master data)
...
### Automation Opportunity
- auto-count (target sku count and facing requirements)
- auto-sort (uses the assortment matrix (w299) to)
- auto-validate (brushes). system validates: (a) total sku)
...
### Automation Opportunity
- auto-create (— recently created sku, not yet)
- auto-review (next assortment review; discontinued: removed from)
- auto-document (— requires documented reason code (w301).)
...
### Automation Opportunity
- auto-file (resolution, maximum file size, required metadata)
- auto-capture (google merchant center requirements (linked to)
- auto-validate (with a checklist of required corrections.)
...
### Automation Opportunity
- auto-update (defines or updates an asset class)
- auto-review (tax manager reviews the depreciation keys)
- auto-count (appropriate gl accounts (asset, accumulated depreciation,)
