...
- auto-submit (portal)**: vendor submits a new item)
- auto-review (**commercial review & categorization**: category)
- auto-log (planner adds logistical attributes (dimensions, weight,)
...
### Automation Opportunity
- auto-count (prevent dummy account creation and immediate)
- auto-file (potential duplicate profiles)
- auto-review (data steward reviews the "potential duplicates")
...
### Automation Opportunity
- auto-submit (request**: operations submits a location master)
- auto-capture (the cost center, profit center, and)
- auto-log (**logistical setup**: supply chain)
...
### Automation Opportunity
- auto-capture (is initially entered during onboarding (w36))
- auto-file (duplicate vendor profiles or parent-child relationships.)
- auto-review (data analyst reviews flagged potential duplicate)
...
### Automation Opportunity
- auto-report (analyst assesses reporting impact and legacy)
- rule-based auto-approval (vp merchandising approves the new multi-level)
- auto-create (data analyst creates the new nodes)
...
### Automation Opportunity
- auto-validate (quality (dq) checks against predefined rules)
- auto-generate (tin) automatically generate alert tickets routed)
- auto-record (bulk or single-record corrections based on)

### Controls
- CTL-50 (prevent unauthorized master data changes.); CTL-51 (detect stale or incomplete master data.)
operational: bloat causing slow queries if historical/obsolete data is not actively archived
...
| **Volume** | ~650 new employee records/year; ~200 cross-entity transfers/year; 6,757 records under governance |
...
### Automation Opportunity
- auto-capture (hr specialist enters employee master data.)
- auto-validate (master). system validates that the cost)
- auto-flag (unique. system flags employees with missing)
...
### Automation Opportunity
- auto-request (**service item request**: services manager or)
- auto-review (& margin review**: pricing analyst reviews)
- auto-assign (installation), subcontractor assignment, warranty coverage, and)
...
| **Volume** | ~15–20 active bank accounts across 5 entities and 4 banking partners (BDO, BPI, Metrobank, Chinabank); ~200 store deposit points; ~5 e-wallet settlement accounts |
...
### Automation Opportunity
- auto-record (**bank master record creation**: treasury manager)
- auto-count (**entity bank account setup**: treasury manager)
- auto-validate (type (cash, check, e-wallet settlement), and)
...
### Automation Opportunity
- workflow orchestration (and geographic coordinates (centroid for distance/routing)
- workflow notification (and lgu contact details. this drives)
- auto-review (areas are reviewed when new dcs)

### Controls
operational: needs the specific barangay), causing data quality degradation in the customer master
...
### Automation Opportunity
- auto-create (data analyst creates the equipment master)
- auto-validate (200-hour forklift check).)
- auto-schedule (the pm schedules and ties them)
...
### Automation Opportunity
- auto-submit (fleet supervisor submits vehicle details (plate)
- auto-log (**compliance data logging**: master data analyst)
- auto-route (defining its route restrictions (e.g., truck)
...
### Automation Opportunity
- auto-scan (contract administrator scans and uploads the)
- auto-extract (category manager extracts commercial terms (e.g.,)
- auto-count (**lease accounting linkage**: for real)
...
### Automation Opportunity
- auto-file (new competitor profile, including their store)
- auto-match (competitor skus (matching by gtin, brand,)
- auto-update (data analyst updates the competitive price)
...
### Automation Opportunity
- auto-scan (hardware profile (scanner, receipt printer, cash)
- auto-generate (vat/lgu tax printing on receipts.)
- auto-file (standard ui profile to the terminal,)
...
### Automation Opportunity
- auto-log (prompt. system logs the consent boolean)
- auto-sync (is immediately synced to the central)
- auto-generate (daily to generate dynamic suppression lists,)
...
### Automation Opportunity
- auto-update (defines or updates the standard greenhouse)
- auto-calculate ((w287) that calculate a vendor's sustainability)
- auto-generate (calculated carbon footprints use the single)
...
- **Store-level variation complexity**: not all 200 stores carry all seasonal SKUs (e.g., garden plants only at 150 stores with garden centers; winter products only in Baguio/Benguet stores); store listing accuracy per seasonal SKU adds complexity to the master data setup
...
### Automation Opportunity
- auto-submit (category manager submits seasonal sku list)
- auto-capture (with garden center vs. coastal stores)
- auto-schedule ((ii) markdown schedule (if not already)
