...
- auto-create (selected store; creates store pick list)
- auto-generate (alert + printed pick list))
- auto-scan (or backroom; scans each item to)
...
### Automation Opportunity
- auto-review (supply planner reviews split fulfillment frequency)
- auto-assign (pick task; assigns to picker by)
- auto-scan (picks items; scans each for confirmation)
...
### Automation Opportunity
- auto-validate (item, system checks atp at stores)
- auto-review (dc dispatch reviews ship-from-store suggestion; confirms)
- auto-record (outbound shipment record)
...
### Automation Opportunity
- auto-assign (routing engine assigns orders to the)
- auto-dispatch (**dispatch**: 3pl couriers (lalamove/transportify))
...
### Automation Opportunity
- auto-capture (or call center; provides reason and)
- auto-review (**validation**: csr reviews request against return)
- auto-generate (home pick-up; generates "return label" for)
...
### Automation Opportunity
- auto-generate (order to generate back-to-back po)
- auto-capture (co-branded carrier; enters tracking number in)
- auto-track (automatically imports tracking status via carrier)
...
### Automation Opportunity
- auto-count (**expedited pick-up counter** rather than the)
- auto-scan (smart locker; scans the locker qr)
- auto-capture (scans the qr/enters the pin at)
...
### Automation Opportunity
- auto-calculate ((automated)**: system calculates sla metrics for)
- auto-review (generation and review**: fulfillment analyst reviews)
- auto-log (complaint; action: log for trend analysis,)
...
### Automation Opportunity
- auto-scan (dispatch coordinator scans shipment barcode and)
- auto-generate (notification**: system generates unique tracking link)
- auto-send (updates**: system sends milestone sms/email to)
...
### Automation Opportunity
- auto-create (per pos-021: creates sub-orders with separate)
- auto-send (notification**: system sends w708 communication within)
- auto-route (dcs**: system routes sub-orders to respective)
...
### Automation Opportunity
- auto-log (customer on logistics needs: (i) delivery)
- auto-calculate ((b) system calculates delivery cost per)
- auto-generate (dc: system generates transfer request to)
...
### Automation Opportunity
- auto-document (proper pod documentation (photo + signature))
- auto-log (ranking**: ecommerce logistics manager reviews dashboard)
- auto-review (next 2-week review cycle; (e) if)
...
### Automation Opportunity
- auto-validate (delivery at checkout, pays express delivery)
- auto-scan ((c) items scanned for accuracy and)
- auto-dispatch (**express courier dispatch & delivery**: (a))

### Controls
operational: shows stock but physical shelf is empty, order fails
### Staffing Implication
- **Store Fulfillment Associate**: Express pick & pack adds ~20–30 min per order; at ~1–3 express orders/store/day, ~30–90 min/day — absorbed by existing stock associates
...
| **Frequency** | ~30–40% of ecommerce orders are mixed-basket (multi-origin) |
...
### Automation Opportunity
- auto-validate (placement: system checks inventory availability across)
- auto-create (creation**: system creates parent order with)
- auto-schedule (delivery: system schedules deliveries to arrive)
...
| **Frequency** | ~8,000–12,000 ship-from-store orders/month (stores serve as fulfillment nodes for bulky and local-availability items) |
| **Volume** | ~40–60 ship-from-store orders/store/month; peak during sale events and Ber months |
| **Owner** | Ecommerce Operations Manager |
...
### Automation Opportunity
- auto-route (ecommerce system routes order to nearest)
- auto-generate (assignment**: system generates digital pick list)
- auto-scan (using handheld scanner; scans each item)
...
- **3PL rider availability** — on-demand riders are shared resources; during peak hours (lunch, end of business day), rider availability drops, risking SLA breach; backup 3PL contracts are essential
...
### Automation Opportunity
- auto-validate (inventory: (a) check real-time inventory per)
- auto-flag (pick list (flagged as express): (a))
- auto-count (customer service counter; (d) stock associate)
...
| **Frequency** | ~8,500–14,300 ecommerce returns/month (ecommerce return rate of ~20–25% of orders; ~42,900 orders/month × 20–25%) |
| **Volume** | ~40–70 in-store ecommerce return drop-offs per store per month; ~8,500–14,300 chain-wide/month |
...
### Automation Opportunity
- auto-count (customer service counter with item to)
- auto-validate (quantity), (c) checks return eligibility: (i))
- auto-record (procedures; csr records grade in system)

### Controls
operational: Customer Service Representative (CSR) review and approval gate
### Time Estimate
...
- Monthly per store: ~50 returns × 20 min = ~17 hours/month per store
- Monthly chain-wide: ~10,000 returns × 20 min = ~3,300 hours/month
- Annual: ~40,000 hours/year

### Staffing Implication
- **CSR**: ~17 hours/month on ecommerce returns per store. This is a significant portion of the CSR's time (1 CSR per store, ~170 working hours/month); at ~10% of CSR time on ecommerce returns, it is absorbed but leaves limited capacity for other CSR duties (BOPIS pickup, special orders, complaints). Consider part-time CSR support during post-sale return spikes.
- **No permanent incremental headcount.**
...
| 5 | **Weekly SLA performance review**: (a) E-Commerce Operations Manager reviews weekly BOPIS SLA metrics: (i) chain-wide SLA compliance rate (target: ≥ 95%), (ii) SLA compliance by store (identify bottom 10 stores), (iii) SLA compliance by day of week (payday weekends vs. weekdays), (iv) top reasons for SLA breach (staffing, stockout, volume), (b) for stores with < 90% SLA compliance: Store Manager must submit corrective action plan (e.g., assign dedicated BOPIS picker during peak hours), (c) trend analysis: is SLA compliance improving or deteriorating? | E-Commerce Operations Manager | VP Digital Commerce | 2–3 hours/week |
...
### Automation Opportunity
- auto-route ((a) system routes bopis order to)
- auto-escalate ((c) system auto-escalates: (i) t+2 hours)
- auto-log (notification with apology)
...
| **Frequency** | Continuous as new SKUs onboard (~200–300 new SKUs/month); quarterly SEO review; monthly content audit |
| **Volume** | 35,000 active SKUs on ecommerce platform; ~200–300 new SKUs/month requiring content; ~5,000 existing SKUs with below-benchmark conversion requiring enrichment per quarter |
...
### Automation Opportunity
- auto-review (seo specialist reviews title, description, and)
- auto-log (published on buildright.com.ph/blog for organic search)
- auto-flag (average — flag for enrichment, (d))
...
### Automation Opportunity
- auto-send (reminders**: system sends automated reminders to)
- auto-generate ((c) system generates daily auto-cancel report:)
- auto-log (phone: (i) apologize for any inconvenience,)
...
- Restocking: 7 min × 2,500 orders/month ÷ 200 stores = ~7 min/store/day = absorbed
