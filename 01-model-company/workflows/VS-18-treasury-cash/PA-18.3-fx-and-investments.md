...
- **Counterparty bank credit risk concentration**: forward contracts are placed with 4 banks (BDO, BPI, Metrobank, Chinabank); if any bank experiences credit deterioration, the MTM gain on outstanding contracts with that bank becomes uncollectible; diversification is limited by the small Philippine banking market


### Automation Opportunity
- auto-generate (identification**: system generates fx exposure report)
- auto-review (treasury analyst reviews fx exposure report;)
- auto-count (from 2–3 counterparty banks for required)
...
### Automation Opportunity
- auto-review (treasury analyst reviews weekly cash flow)
- auto-count (special deposit account (sda) with bsp-qualified)
- rule-based auto-approval (treasury manager approves; (b) php 50m–200m:)
...
### Automation Opportunity
- auto-track (maintenance**: system tracks all active credit)
- auto-generate (treasury manager prepares drawdown request: amount,)
- rule-based auto-approval (50m: cfo approves, (b) php 50m–200m:)

### Controls
- CTL-61 (ensure debt covenant monitoring.)
operational: Fixing Rate) or PHIBOR + spread, rising benchmark rates increase interest expense; operational: document can trigger a technical default even when financial covenants are met
...
### Automation Opportunity
- auto-count (balances (usd accounts per entity), (d))
- auto-import ((30–60 day) import payment obligations, (b))
- auto-forecast (coverage of forecasted usd payables); identifies)
...
### Automation Opportunity
- auto-submit (subsidiary guarantees) submits guarantee request: type,)
- auto-count (by a counter-guarantee or holdback? (e.g.,)
- rule-based auto-approval (guarantee: cfo approves; (b) php 50m–200m:)
...
- auto-record ((g) treasury records lc in erp)
- auto-submit ((b) treasury submits lc amendment to)
...
| 5 | **Quarterly transfer pricing rate review**: Tax Accountant and Finance Controller review IC FX rate methodology quarterly; verify BSP rate vs. actual settlement rates (variance < 1%); document rate methodology per RR 19-2020 | Tax Accountant | Finance Controller | 4 hours/quarter |
| 6 | **Annual transfer pricing documentation**: Tax Accountant prepares annual TP documentation per RR 19-2020: transaction summary, pricing method, benchmarking analysis, arm's-length justification | Tax Accountant | VP Legal / CFO | 20–30 hours/year |
...
### Pain Points / Risks
- **BIR transfer pricing audit exposure**: inconsistent FX rates across IC settlements create audit risk under RR 19-2020
...
### Automation Opportunity
- auto-generate (aggregation**: system generates monthly ic payable/receivable)
- rule-based auto-approval (analyst applies approved rate: bsp spot)
- auto-post (cross-bank); system posts ic settlement entries)
...
### Pain Points / Risks
- **PFRS 9 complexity** — hedge accounting under PFRS 9 requires extensive documentation (hedging relationship designation, risk management objective, effectiveness testing methodology); non-compliance results in all derivative changes hitting P&L, creating earnings volatility
...
- **Auditor scrutiny** — hedging is a high-risk audit area; External Auditor requires full documentation trail for each contract from inception through settlement


### Automation Opportunity
- auto-record (contracts; (d) records mtm adjustment in)
- auto-document (original hedge documentation (hedged item, risk,)
- auto-match (settled, timing mismatch); (c) prospective effectiveness:)
...
### Pain Points / Risks
- **PDIC insurance gap** — PDIC covers only PHP 500K per depositor per bank; BuildRight's PHP 500M–1B portfolio is largely uninsured; counterparty risk management is the primary protection
...
### Automation Opportunity
- auto-review (**monthly portfolio review**: (a) treasury analyst)
- auto-validate (placement: (i) verify counterparty risk acceptable)
- auto-count (**counterparty risk assessment**: (a))

### Controls
operational: Treasury Manager review and approval gate

### Staffing Implication
- **Treasury Analyst**: ~2 hours/month on portfolio monitoring + 30 min/maturity × ~8/month + 4 hours/quarter on counterparty review = ~60 hours/year; absorbed
