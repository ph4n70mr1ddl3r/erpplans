...
- auto-generate (store managers prepare daily cash deposit)
- auto-count (all bank accounts (bdo, bpi, metrobank,)
- auto-review (treasury analyst reviews daily cash position)
...
### Automation Opportunity
- auto-forecast (add manual forecasts for capex, loan)
- auto-identify (**analysis**: identify liquidity gaps or)
- auto-report (**reporting**: weekly cash position)
...
### Automation Opportunity
- auto-match (**reconciliation**: match intercompany sales/purchases and)
- auto-log (bought from logistics inc.))
- auto-post (**elimination entries**: post elimination journals in)
...
### Automation Opportunity
- auto-log (intercompany markups (logistics services, rent))
- auto-document ("transfer pricing documentation" (tpd) per bir)
- auto-review (**quarterly monitoring**: review actual intercompany margins)
...
### Automation Opportunity
- auto-document (treasury policy document covering: (a) investment)
- auto-count ((e) maximum single-counterparty credit exposure as)
- auto-review (committee; committee reviews and recommends to)
...
### Automation Opportunity
- auto-review (treasury analyst reviews overnight bank balances)
- auto-count (— store accounts**: system automatically initiates)
- auto-log (entity (e.g., logistics inc.) has surplus)
...
### Automation Opportunity
- auto-count (or dynamic discounting based on: (a))
- auto-review ((b) legal reviews scf agreement (per)
- auto-match (w7 3-way match), system evaluates —)
...
### Automation Opportunity
- auto-count (all ~210 accounts; (b) flag any)
- auto-calculate (treasury analyst calculates and posts month-end)
- auto-validate (payable); (c) verify that monthly interest)
...
### Automation Opportunity
- auto-validate (— (a) verify retained earnings availability)
- auto-record (share, (b) record date (date determining)
- auto-review (of directors reviews and approves dividend)
...
### Automation Opportunity
- auto-capture (signature present); enters pdc into erp)
- auto-validate (transfers physical check to store/hq vault)
- auto-count (gl cash account is not yet)
...
### Automation Opportunity
- auto-generate (months); system prints checks with future)
- auto-validate (**signatory approval**: checks are routed for)
- auto-record (**pdc issuance recording**: treasury analyst records)
...
### Automation Opportunity
- auto-validate (the returned check)
- auto-count (re-instated (dr. accounts receivable / cr.)
- auto-generate (ar clerk generates a debit memo)
...
| 4 | **Payroll forecast**: Payroll Manager provides payroll forecast for 4-week window — semi-monthly payroll on the 15th and 30th (~PHP 600M per run covering ~8,500 employees); variable components: overtime, commissions, 13th month accrual release (November–December), statutory contributions (SSS, PhilHealth, Pag-IBIG) due by last working day of month; special payroll runs (separation pay, backpay adjustments) | Payroll Manager | Treasury Analyst | 10 min |
...
### Automation Opportunity
- auto-document (week's forecast; documents material variances (>±5%)
- auto-review (treasury analyst reviews collection efficiency —)
- auto-count (w556; early-pay discount opportunities (2/10 net)
...
### Automation Opportunity
- auto-forecast (month against forecast per w589: cash)
- auto-import (impact on import payments)
- auto-update (treasury manager updates rolling 4-week forecast)
...
### Automation Opportunity
- auto-record (per system); record count on dual-signature)
- auto-count (denomination, (b) counts and bands cash)
- auto-validate (guard jointly verify: (a) number of)
