...
- auto-submit (hiring manager submits staffing request with)
- auto-post (recruitment officer posts job on job)
- auto-record (pipeline stage records date, recruiter notes,)
...
| **Volume** | ~6,757 employees; all employees reviewed annually |
...
- **Total distributed effort**: ~6,000 hours/year across ~230 managers for ~6,757 employees
...
### Automation Opportunity
- auto-capture (cross-training); goals entered in system with)
- auto-document (check-in notes documented in system)
- auto-review (**annual performance review** (december): manager completes)

### Controls
operational: must enforce mandatory documentation at each PIP milestone
### Staffing Implication
- **No incremental headcount.** Performance reviews are distributed across ~230 managers (Store Managers, Dept. Supervisors, Dept. Heads). Annual cycle adds ~45 min/employee/year for ~6,757 employees = ~6,000 hours total effort, distributed across the management team.
...
### Automation Opportunity
- auto-capture (supplies, training, entertainment, other), amount, business)
- auto-validate (system validates claim: (a) receipt)
- auto-route (system routes claim for approval)
...
### Automation Opportunity
- auto-submit (employee submits loan/advance request in)
- auto-document (salary; requires documented purpose (medical, education,)
- auto-record (creates loan record in system: principal)
...
| **Volume** | ~6,757 employees; ~20,700 sets of uniforms (3 per employee) + safety gear |
| **Owner** | HR Operations Manager |
| **Participants** | HR Assistant, Store/DC Manager, Procurement, Vendor, Employee |

### Background

With over 6,757 employees in retail and distribution, maintaining a consistent professional image and ensuring safety compliance (PPE) is a significant logistical task. BuildRight provides uniforms and mandatory safety gear (safety shoes, helmets, vests) to all store and DC personnel.
...
### Automation Opportunity
- auto-assign (shoes); system assigns standard entitlement (e.g.,)
- auto-count (id for accountability)
- auto-track (**inventory tracking**: system tracks ppe/uniform)
...
### Automation Opportunity
- auto-review (identification**: annual review of performance (w72))
- auto-identify (**succession mapping**: identify "ready now" and)
- auto-assign ((idp)**: successors assigned to specific training)
...
### Automation Opportunity
- auto-capture (and assessment centers targeting fresh graduates)
- auto-assign (**project assignment**: trainees assigned a)
...
| **Volume** | 6,757 employee base; average ~120–180 sickness claims/month, ~40–60 maternity leaves/month, and ~250–350 loan applications/month |
...
### Automation Opportunity
- auto-document (loan request documents to the hris/erp)
- auto-review (benefits specialist reviews the uploaded documents;)
- auto-log (benefits specialist logs into the government)
...
### Automation Opportunity
- auto-submit (vendor submits promodizer credentials and)
- auto-create (store admin creates a non-employee profile)
- auto-file (biometric attendance profile.)
...
### Automation Opportunity
- auto-count (declaration**: vendor account manager submits the)
- auto-track (the "incentive tracking module" (linked to)
- auto-generate (validation**: system generates a "promodizer sales)
...
### Automation Opportunity
- auto-review (**desk audit**: review current promodizer list)
- auto-document (**documentation review**: verify that)
- auto-request ("corrective action request" to store managers)

### Controls
operational: regularization of thousands of workers, impacting financial sustainability
### Time Estimate
...
### Automation Opportunity
- auto-monitor (practice); (e) monitor employee sentiment indicators)
- auto-generate (coercion); (e) prepare management's position for)
- auto-document (union loses: document outcome and continue)
...
| **Volume** | 6,757 employees across all locations |
| **Owner** | CHRO |
...
- **Geographic coverage**: Providing equitable wellness services to 6,757 employees across 200 stores in provincial and island locations is logistically challenging; health screening and face-to-face counseling require physical presence
...
### Automation Opportunity
- auto-communication (approval; (f) communicate program launch to)
- auto-record (separate confidential records per data privacy)
- workflow orchestration (bmi) — coordinate with dole-required annual)
...
- **External EAP Provider**: Outsourced. Budgeted as employee benefit cost (~PHP 300–500/employee/year × 6,757 = ~PHP 2.0–3.4M/year).
...
### Automation Opportunity
- auto-submit (initiated: employee submits transfer request via)
- auto-log (depot → logistics); (d) cross-entity and)
- auto-validate (performs eligibility checks: (a) probation period)
...
### Automation Opportunity
- auto-review (agenda: (a) review of previous meeting)
- auto-log (with attendance log)
- auto-inspect (reviews safety inspection findings (w141): open)
...
### Automation Opportunity
- auto-count (to hr: headcount by role (sales)
- auto-validate (business partner validates request against seasonal)
- auto-document (rules, and documentation requirements)
...
| **Volume** | ~6,757 employees across 200 stores, 4 DCs, HQ |
...
### Automation Opportunity
- auto-send (weeks), and sends launch communication)
- auto-remind (and sms; reminder sent at day)
- auto-track ((no ip tracking, no timestamps linked)
...
### Automation Opportunity
- auto-send (any employee sends peer recognition via)
- auto-validate (customer service (validated by customer feedback)
- auto-review (selection committee review; select winners per)

### Controls
operational: spot award budget allocation per store (PHP 10,000/month) and system-tracked distribution analytics; operational: supervisor co-sign requirement for recognitions > 5 per month per employee and quality spot-check by HR; operational: monthly budget tracking and approval workflow for awards exceeding threshold
### Staffing Implication
...
- **Frequency**: ~100–130 enrollment/changes/month (new hires + events); annual renewal for 6,757 employees
- **Volume**: ~6,757 covered employees + ~2,000 dependents across 5 entities
- **Owner**: HR Benefits Specialist
...
- **Automation Opportunity**: Auto-enrollment triggered from onboarding workflow; HMO roster auto-reconciled against employee master with exception queue; dependent-document OCR validation; utilization-report auto-generation from HMO provider feed
- **Controls**: operational: HR Manager approval gate for enrollment changes; monthly roster reconciliation; annual competitive bidding with ≥2 alternative HMO providers
...
- **Automation Opportunity**: Self-service portal with AI chatbot for Tier-1 FAQ resolution; auto-ticket classification and routing; knowledge-base auto-suggestion from resolved tickets; SLA auto-escalation
- **Controls**: operational: HR Service Desk Lead weekly ticket audit; satisfaction-survey feedback loop; knowledge-base review-and-approval gate before publication
...
- **Volume**: ~6,757 active employee records + ~100,000 former applicant records + ~20,000 former employee records under retention
...
- **Automation Opportunity**: Auto-DSAR fulfillment with cross-system data aggregation; auto-consent-validity tracking and expiry alerts; auto-retention-schedule enforcement with purge workflows; DPIA auto-generation from system inventory
- **Controls**: operational: DPO quarterly compliance review; 72-hour breach notification protocol; NPC registration annual renewal; Data Processing Agreements with all third-party HR data processors
...
### Automation Opportunity
- auto-validate (hr coordinator checks if position is)
- auto-post (hr coordinator posts position on internal)
- auto-submit (employee submits internal application with)
...
| **Frequency** | Ongoing (~100–150 referrals/month across 6,757 employees) |
...
### Automation Opportunity
- auto-capture (postings; (b) enters candidate name, contact)
- auto-validate (days: (a) check if candidate already)
- auto-submit (referral status: submitted → validated →)
...
| **Volume** | ~10–15 internal communications/week across 6,757 employees in 205+ locations |
...
### Automation Opportunity
- auto-submit (regions; (d) submits content draft to)
- auto-review (communication: (a) reviews content for accuracy,)
- rule-based auto-approval (w134; (e) approved content queued for)
...
### Automation Opportunity
- auto-review (store: (a) review store crime statistics)
- auto-record (employee training record per w51)
- auto-document (**incident documentation**: within 2 hours)
...
### Automation Opportunity
- rule-based auto-approval (transfer is approved per w511: (a))
- auto-record ((d) budget recorded in system linked)
- auto-log (**logistics coordination**: hr manager)
...
- **Pay equity analysis complexity**: comparing compensation across 6,757 employees in 5 entities with varying regional wage orders and role-specific allowances requires careful normalization to avoid misleading conclusions
...
### Automation Opportunity
- auto-identify (same role/grade; identify unexplained pay gaps;)
- auto-count ((d) assign accountability to department heads)
- auto-review (**policy review and enforcement**: semi-annually:)
...
| **Volume** | ~200 new hires chain-wide at any given time (based on annual turnover ~40% × 6,757 employees / 12 months) |
...
### Automation Opportunity
- auto-validate (day-7 check-in — department supervisor)
- auto-create (area: supervisor creates targeted support plan:)
- auto-record (and punctuality record (w561), (d) progress)
...
### Automation Opportunity
- auto-report (or reminder: reporting procedures, policy highlights,)
- rule-based authorization ((b) no unauthorized voids (w529), (c))
- auto-log (stockroom access log reviewed for unusual)
...
### Automation Opportunity
- auto-submit (submission**: employee submits leave request via)
- auto-validate (and eligibility check**: system validates: (a))
- auto-document (supporting medical documentation upload, (d) system)
...
| **Volume** | 6,757 employees × annual enrollment + ~200 qualifying life event changes/year |
...
### Automation Opportunity
- auto-review (admin: (a) reviews current plan utilization)
- auto-document ((d) faq document and hotline number,)
- auto-submit (reviews and submits elections via self-service)
...
Annual enrollment period: Preparation (2-3 days) + communication (4 hours) + enrollment processing (~40 hours for 6,757 employees) + verification (~20 hours) + transmission (4 hours) + audit (4 hours) = ~80–90 hours concentrated in 4-6 weeks. Ad-hoc qualifying events: ~20 min each × 200/year = ~67 hours distributed throughout year.
...
### Automation Opportunity
- auto-assign (id), (b) assigns safety shoes (steel-toe)
- auto-generate ((a) system generates uniform replacement schedule)
- auto-validate (— wear/tear check (replace when holes)
...
### Automation Opportunity
- auto-submit (**employee submits travel request via)
- auto-validate (travel policy check**: erp validates request)
- auto-capture (department cost center, (d) approves, rejects)
...
### Automation Opportunity
- auto-count (conducts employee headcount within 24 hours)
- auto-log (receives and logs claim; (d) system)
- auto-validate (hr department validates: (a) damage severity)
...
### Pain Points / Risks
- **6-month regularization risk**: if a seasonal worker's engagement extends beyond 6 months (e.g., holiday season extends into January clearance sale), DOLE considers the employee regularized — entitled to all regular benefits, cannot be separated without just cause and due process; Store Manager must strictly enforce end dates
...
### Automation Opportunity
- auto-submit (impact; (b) submits request to hr)
- auto-record (no criminal record; (c) interview and)
- auto-generate (business partner prepares employment contract: (i))
...
*Workflow Count: 35 · Back to **[VS-19: Hire-to-Retire](./README.md)** · [Value Stream Index](../value-stream-index.md)*
