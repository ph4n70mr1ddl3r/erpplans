...
- auto-calculate (system calculates rop breaches based)
- auto-generate (system generates suggested pos grouped)
- auto-review (buyer reviews suggested po list)
...
### Automation Opportunity
- auto-schedule (quantities, delivery schedule, payment terms, rebate)
- rule-based auto-approval (category manager approves contract terms; vp)
- auto-validate (finance validates contract commitment against)
...
### Automation Opportunity
- auto-count (store (csr counter) or via sales)
- auto-validate (system: (a) checks if item exists)
- auto-route (system routes non-stock item creation)
...
### Automation Opportunity
- auto-request (requestor identifies emergency need)
- auto-submit (requestor submits emergency procurement request)
- auto-log (hours; system logs verbal approval with)
...
### Automation Opportunity
- auto-review (legal reviews contract terms: liability,)
- auto-record (vendor contract record: vendor, contract type,)
- auto-alert (system alerts dept. head 90)
...
### Automation Opportunity
- auto-match (receipt & auto-match to po**: vendor)
- auto-review (for buyer review; auto-generates vendor notification)
- auto-log ((d) resolution logged in system with)
...
### Automation Opportunity
- auto-capture ((d) buyer enters discontinuation notice into)
- auto-count (identify trade accounts with recurring purchase)
- auto-calculate (merchandise planner calculates ltb: (a) ltb)

### Controls
operational: are stranded
### Staffing Implication
- **Buyer**: Each discontinuation event requires 15–30 minutes of administrative processing; at 30–50 events/month across the Buyer team (10–12 Buyers), this is ~2–3 hours/month per Buyer; absorbed by existing roles
- **Category Manager**: Impact assessment and transition decisions require 1–3 hours per event; at 30–50 events/month across 5 Category Managers, this is ~6–10 hours/month per Category Manager; absorbed by existing roles
- **No incremental headcount**
...
| **Frequency** | ~400–600 partial shipments/month (~30–40% of merchandise POs have at least one partial shipment) |
...
### Automation Opportunity
- auto-update ((b) system updates po: shipped lines)
- auto-scan (against po: scan each received line,)
- auto-review ((b) buyer reviews backorder report daily)

### Controls
operational: Buyer review and approval gate
### Staffing Implication
- **Buyer**: Backorder management adds ~1–2 hours/day to existing workload; with 10–12 buyers, each manages ~30–50 backordered lines — absorbed by existing team with improved tooling
...
| **Volume** | ~25–35 documentation packages/month; peak during pre-season build-up (Feb–Mar, Sep–Oct) |
...
BuildRight imports ~40% of COGS from international vendors (China, Taiwan, Indonesia, Malaysia, Japan, Europe), requiring ~400–600 TEUs/month of containerized shipments. Philippine customs clearance requires a comprehensive documentation package for each import entry, and any missing or incorrect document can result in container detention at the port, demurrage charges (PHP 5,000–15,000/container/day per W249), customs examination, or seizure. The Philippines Bureau of Customs (BOC) operates under CAO 4-2016 (Customs Modernization and Tariff Act), and import processing is facilitated through the E2M Customs system. This workflow ensures all required documents are prepared, verified, and submitted before vessel arrival.
...
- **HS code misclassification risk**: BuildRight imports 35,000 SKUs across 13 categories; incorrect HS code classification can result in duty underpayment (BOC audit liability up to 5 years back + penalties) or overpayment (permanent cash flow loss); Customs Broker expertise is critical
...
### Automation Opportunity
- auto-document (of credit documents — if lc)
- auto-send (required to send documents within 48)
- auto-log (broker coordinator logs receipt date and)
...
### Staffing Implication
- Customs Broker Coordinator (1 FTE): ~80 hours/month is a near-full-time role; this is a dedicated position within the Supply Chain or Finance team
...
*Workflow Count: 9 · Back to **[VS-03: Vendor Management & Procurement](./README.md)** · [Value Stream Index](../value-stream-index.md)*
