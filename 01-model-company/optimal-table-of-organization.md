...
> The **target state** is this document: **HQ 469 / total 6,869** at 200 stores, reached
...
> IT = 80 FTE; VS-177 field retail operating model), and each phase's promotion is
...
| **Target (this document)** | **469** | **6,869** | Adopted 2026-09-02; optimal steady state at 200 stores; phased per §11; scales per §12 |

**Design stance.** The current baseline (HQ 362) is the *minimum-viable* TO — the floor that
resolves the known gaps. The adopted target lands at **HQ 469** — the upper edge of the
reality-check's comfortable band (~440–470, pushed to the top by IT = 80 per the product
...
7. **Field-first, HQ-lean.** HQ designs and standardizes; the field executes and feeds back.
   ...
   HQ-to-field ratio ≈ 7% (469 : 6,400), inside the 6–10% big-box norm.
...
```
   ...
   │
   ├── CIO ──────────────────────────── Information Technology (7 domain + 5 platform product
   │                                      teams + CIO Office = 80 FTE; DPO administrative line)
   │
...
│         Domain: MSC · WLI · SSP · CCP · FIN · CORP · PEO
│         Platform: IAP · INFRA · SEC · DP · FS (+ CIO Office)
├── CMO
...
| CIO | 2 (Head of EA / portfolio+FinOps) via CIO Office; 12 product teams | 80 | ✅ (platform-led span) |
| CMO | 1 + dotted GM Digital Commerce Inc. | 30 | ✅ light span absorbs retail-media growth |
| CHRO | 2 | 55 | ✅ |
| VP Legal & Compliance | 2 + CorpSec (statutory) | 24 | ✅ |
| **Total** | **18 departments + Executive Office** | **462 + 7 = 469** | |
...
| 5 | Information Technology | 50 | 65–80 | **80** | 12 product teams per it-product-operating-model.md §9.1 (authoritative) |
...
| | **Total company** | **6,762** | | **6,869** | Revenue/employee ≈ PHP 9.07M |

> 469 lands inside the reality-check comfortable band (~440–470) at its upper edge — the
> increment over ~455 is IT at 80 (top of its band), sized independently and authoritatively
> by the IT product operating model §9.2.
...
**Information Technology (80)** — governed by the IT product operating model, not restated
here: 7 stream-aligned domain teams paired to business owners, 5 platform teams (integration,
infra, security/GRC, data, field services), CIO Office. Target = 80 (already the
authoritative steady-state sizing), top of need band.
...
| **HQ : field ratio** | 6–10% | 469 : 6,400 = **7.3%** | ✅ |
...
## 11. Sizing & Phasing — Current 362 → Target 469
...
| **3 — Capability scale** | 18–36 mo | IT 50→80 (per product-model sequencing); HR 26→42 (field HRBPs as regions mature); SC 40→46; Merch 40→43; Marketing 25→30; LP 20→27; Facilities 8→12; Quality 4→5; ESG/Strategy/Trade top-ups | +63 |
| | | **Total** | **362 → 469 (+107)** |

**Guardrails.** Total HQ never exceeds the 440–470 band at 200 stores; any phase that would
...
| HR / CS / Finance / IT | 42 / 34 / 62 / 80 | 52 / 45 / 70 / 95 | HQ grows at ≤ half the store rate via ERP automation |
| **HQ total** | **469** | **~530** | discipline: 7% → ~5% of company |
| **Company total** | **6,869** | **~9,800** | revenue/employee ≥ PHP 9M preserved |
