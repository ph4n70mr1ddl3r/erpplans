...
## 2026-06-21 — Quality review: honest-ize Automation/Controls draft fields, backfill CTL-XX, fix repo hygiene, add validator Check 21

A whole-repo quality review (distinct from the prior consistency reviews, which validated cross-reference *integrity*). Where reviews #1–#9 confirmed that every workflow *has* the two standard analysis fields, this review audited the **content quality** underneath those field headers and the repo's *hygiene*. It found the Automation/Controls fields were added repo-wide by a first-pass generator whose output is overwhelmingly draft quality, and that the docs/validator were reporting this as "100% complete" — overstating maturity. The review also surfaced tracked build artifacts, missing `.gitignore`, and a blank-line formatting defect. All are corrected below. `validate-repo.sh` now reports **0 errors / 2 informational warnings** across all **21** checks (the new Check 21 adds the second warning, tracking the draft-field backlog).
...
**1. Added validator Check 21 — Automation/Controls content quality.** Checks 10/12/15 guard the fields' *presence* and one legacy boilerplate string, so they could not see that ~72% of the machine-generated Automation bullets were mid-phrase fragments (e.g. `- auto-review (account manager reviews application: (a) verify)`) and ~89% of Controls sections cited no CTL-XX from the controls register. Check 21 reports three live metrics — fragment-bullet count, CTL-XX coverage %, and pure-boilerplate Controls count — as a WARN (not an ERROR), the same treatment Check 1 gives unclassified workflows. This makes the quality bar *measurable* rather than aspirational. Current reading: **5,665/11,217 fragment bullets; 615/5,317 Controls cite a CTL-XX (11%); 541 pure-boilerplate**.

**2. Corrected the misleading "100%" claim to "100% presence, draft content".** `WORKFLOW-FORMAT-GUIDE.md` ("Standard analysis fields"), `validate-repo.sh` Check 12's OK message, and the two READMEs now distinguish *presence* (still 100%) from *content quality* (tracked by Check 21), with a clear caveat that generated Automation/Controls bullets are draft pending per-workflow human review. The format guide's existing ❌-example quality bar is unchanged and remains the target.

**3. Retroactively backfilled real CTL-XX references (`backfill-controls.py`).** New documented, idempotent tool that reverses `internal-controls-matrix.md` into a workflow→control map and injects a properly-formatted `- CTL-XX (objective); ...` bullet into every Controls section that lacks a CTL-XX but has a matrix mapping. Result: **60 Core workflows** (the register's coverage scope is W1–W942) now cite their authoritative control(s); CTL-XX coverage rose from 555 → **615 sections**. The tool also normalized the **3,235 Controls sections** that were missing the blank line before the next `###` header (a rendering defect from the original generation run; 0 remain).

**4. Hardened the generator (`add-automation-controls.py`) for future runs.** Fixed (a) a duplicate `'approve'` key in `MANUAL_VERBS` that Python silently dropped; (b) `generate_automation()` now emits complete, period-terminated sentences (`System {auto-action} of "{step summary}" (replaces manual Step N).`) instead of mid-phrase fragments, using a boundary-respecting step summary; (c) `generate_controls()` is now CTL-map-aware — it cites real CTL-XX from the register where a mapping exists before falling back to pain-point-derived or step-pattern controls. Re-running on the existing 5,317 workflows is intentionally a no-op (they already have the fields); the hardened generator governs *future* workflows.

**5. Repo hygiene.** Added a `.gitignore` (`__pycache__/`, `*.pyc`, `.antigravitycli/`, editor state) and un-tracked two artifacts that had been committed: `07-methodology/__pycache__/classify-workflows.cpython-312.pyc` and `.antigravitycli/efabada1-….json` (CLI tooling state). Cleared the executable bit on 14 documentation `.md` files that carried `+x` from a prior `chmod`.

**Deferred (noted for follow-up, out of scope for this pass):** (a) Per-workflow human refinement of the remaining ~5,665 fragment Automation bullets and ~4,702 CTL-less Controls — this is genuine analytical work per workflow, not a mechanical transform; the hardened generator + Check 21 + the backfiller are the scaffolding for incremental progress. (b) Extending the 67-control register to cover the gap-analysis block (W2993+), which would lift CTL-XX coverage materially. (c) Condensing the long-form consistency-review essays in this file. (d) Splitting the four PA files >450 KB (`VS-09/PA-09.2` 656 KB, `PA-09.1` 524 KB, `VS-07/PA-07.1` 449 KB, `VS-07/PA-07.2` 358 KB) along their existing sub-process boundaries.

---

## 2026-06-21 — Consistency review #9: fix 1 stale PA-file nav link + add validator Check 20 (PA-file link resolution)

A ninth whole-repo consistency + unambiguity review. The eight prior reviews and `validate-repo.sh`'s 19 checks all passed; this pass audited the dimensions the mechanical checks do not cover — every intra-repo markdown link across all 774 documents, cross-document figure/term consistency, and stale prose claims — and surfaced exactly one residual defect, which is fixed below with a new guard check so it cannot recur. `validate-repo.sh` now reports **0 errors / 1 informational warning** across all **20** checks.

**Canonical totals (unchanged):** 187 value streams · 565 process areas · 5,317 workflows · 733 requirements (431 Must / 296 Should / 6 Nice) · 67 controls · W1–W5464 · VS-89–VS-191 · twenty-nine gap-analysis passes. (VS-79–VS-191 = 113 value streams / 2,712 workflows; gap-analysis VS-89–VS-191 = 103 value streams / 2,472 workflows = W2993–W5464 — all four figures verified against the filesystem.)

**1. Fixed 1 broken PA-file navigation link in `VS-56 PA-56.1`.** The header line `> Part of **[VS-56: Third-Party Delivery Partner Management](./README.md)** (Make & Move) · [Value Stream Index](../value-stream.md)` linked `../value-stream.md`, which does not exist; the correct target (used by the other 410 PA-file headers and by this file's own footer) is `../value-stream-index.md`. A whole-repo link-resolution scan of all 7,134 relative/absolute markdown links confirmed this was the **only** genuinely-broken navigational link across all 774 `.md` files: the only other flagged hits sit inside backtick inline-code spans in `CHANGELOG.md` illustrating the standardized footer template (verified as false positives in review #5 and left as literal code). No other files referenced `value-stream.md` (without `-index`), so no further edits were needed.

**2. Added `validate-repo.sh` Check 20 — PA-file relative-link resolution.** Check 19 resolves the intra-repo links inside `value-stream-index.md`, but no check validated the navigation links inside the 565 PA files themselves (a `Part of **[VS-NN: …](./README.md)** … [Value Stream Index](../value-stream-index.md)` header line, a `*… · [Value Stream Index](../value-stream-index.md)*` footer, and occasional in-body cross-reference links) — so the item-#1 drift shipped undetected. Check 20 resolves every relative (`./` or `../`) intra-repo `.md` link inside every PA file against the filesystem (currently **2,262** links) and errors on any that dangles. Verified to catch the item-#1 regression when reintroduced. `README.md` and `07-methodology/README.md` were updated from "19 checks" to "20 checks". (CHANGELOG entries that historically say "19 checks" are left as the contemporaneous record.)

**3. Review scope (no changes needed — verified consistent).** For completeness, the following were audited and found already consistent, so no edits were made: (a) headline figures 187 VS / 565 PA / 5,317 WF reconcile in `README.md`, `workflows/README.md` (Quick Stats: T1 801 / T2 1,549 / T3 426 = 2,776 rows = 2,753 unique + 23 parent/summary), `value-stream-index.md`, `WORKFLOW-FORMAT-GUIDE.md`, `executive-summary.md`, `headcount-reality-check.md`, and `requirement-workflow-matrix.md`; (b) all 187 per-VS counts in the root `README.md` folder-structure tree match the actual PA-file workflow-header counts; (c) all 733 requirements are mapped 1:1 in `requirement-workflow-matrix.md` and reconcile with `erp-requirements.md`; (d) the matrix's "coverage currently limited to core foundational workflows (W1–W942)" claim is accurate (max W-number referenced is W942) — not stale; (e) the dependency-map / touchpoint-map "113 value streams / 2,712 workflows (VS-79–VS-191)" and "twenty-nine gap-analysis passes" claims verify; (f) "205 locations" is defined as 200 stores + 4 DCs + 1 HQ in `model-company-profile.md`; (g) the `CHANGELOG.md` `../value-stream-index.md` hits are confirmed false positives inside code spans.
